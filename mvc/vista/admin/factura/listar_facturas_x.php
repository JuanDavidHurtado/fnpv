...
$where = " where id_usuario='".$_REQUEST['identificacion']."' and estado=0  and date(fecha)='".$fecha_hoy."'";
...
?>
    <form action="?option=listar_facturas_x&search" method="post">

    <?php
    if (@$_REQUEST['term'] != '') {
        ?>
            <a href="?option=listar_facturas_x">Cancelar</a>
        <?php
    }
    ?>
    </form>
...
//".$row['id_factura']."
                ...
                echo "<tr class='$style_file_table'>
                                ...
                                <td align='right'>UNC-" . $row['id_factura'] . "</td>
                                <td align='right'>" . $row['id_eps'] . "</td>
                                <td align='right'>" . $esta_fac_d . "</td>
                                 <td align='center'><a href='?option=buscar_usuario&identificacion=".$_REQUEST['identificacion']."&origen=0&origen33=".$origen33."&id_factura=".$row['id_factura']."&id_cita=".$row['id_cita']."'><img src='" . Utils::$http_ruta . "mvc/vista/imagenes/icon_edit.png' width='18px' height='18px' /></a></td>
    ...
    if ($total_registros) {
        ...
        if (($pagina - 1) > 0) {
            echo "<a href='?option=listar_facturas_x&pagina=" . ($pagina - 1) . "&$term_link'>< Anterior</a> ";
        }
        ...
        for ($i = 1; $i <= $total_paginas; $i++) {
            if ($pagina == $i)
                ...
                echo "<a href='?option=listar_facturas_x&pagina=$i&$term_link'>$i</a> ";
        }

        if (($pagina + 1) <= $total_paginas) {
            echo " <a href='?option=listar_facturas_x&pagina=" . ($pagina + 1) . "&$term_link'>Siguiente ></a>";
        }
