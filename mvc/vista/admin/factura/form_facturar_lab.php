...
$hora_actual_x = date("Y-m-d");

// agregar que la fecha sea la de hoy
if ($_REQUEST['origen33'] == 1) {

    $max_factura1 = "Select * from factura where id_factura='{$_REQUEST['id_factura']}' ";
} else {
    $max_factura1 = "Select max(id_factura) as id_factura from factura where id_usuario='{$_REQUEST['identificacion']}' and origen_factura=1 and fecha_de_factura='{$hora_actual_x}' and estado=0 ";
}
//  $max_factura = "Select max(id_factura) as id_factura from factura where id_usuario='{$_REQUEST['identificacion']}' and  fecha_de_factura='{$hora_actual_x}'  and origen_factura=1 and estado=0 ";
$result_max = $conexion->EjecutarQuery($max_factura1);
...
mensaje();
    ...
    function agregarFila(obj) {
        ...
        var total = $("#total1").val();
           ...
           var f_consul = $("#fecha_consulta").val();
    ...
    //    var strHtml6 = "<td>" + total + '<input type="hidden" id="hdntotal_' + oId + '" name="hdntotal_' + oId + '" value="' + total + '"/><input type="hidden" id="auto_' + oId + '" name="auto_' + oId + '" value="' + auto + '"/></td>';
      ...
      //imagen ....
        ...
        limpiar();
            ...
            alert("Verifique que los campos(EPS,Contrato,Codigo,Cantidad,autorización) no estén vacios")
        }
...
<script type="text/javascript">
    ...
    function limpiar()
    {
        ...
        document.getElementById('autorizacion').value = "";
         document.getElementById('autorizacion_2').value = "";
      ...
      //  var parameters = "opcion=17&id_contrato=" + id_contrato + "&origen=" + origen;
        ...
        if(inputString.length == 0) {
            ...
            $('#suggestions').hide();
              ...
              var parameters = "opcion=89&queryString="+inputString+"&id_contrato=" + id_contrato;
...
<form id="id_factura_formulario" action="?option=detalle_factura&origen=guardar" method="post">
    ...
    <input type="hidden" id="origen" name="origen"  value="<?php echo $_REQUEST['origen']; ?>" />
     <input type="hidden" id="origen33" name="origen33"  value="<?php echo $_REQUEST['origen33']; ?>" />
       <input type="hidden" id="orig" name="ori"  value="<?php echo $id_eps; ?>" />
...
$ep      = $id_eps;
$ret     = '';
$sqlr    = "Select * from eps where id_eps=$ep";
...
?>
  ...
  <table width="340" border="1" align="center" cellpadding="0" cellspacing="0" >
      ...
      <tr>
                ...
                <td colspan="6"><input  name="fecha_consulta" type="text" id="fecha_consulta" class="input_txt " value="0"/></td>
...
<table width="673" border="1" align="center"  cellpadding="0" cellspacing="0" >
        ...
        <tr>
            <td>
                <input  name="fecha_codigo" type="text" id="fecha_codigo" size="7px"  value="<?php echo date("y-m-d") ?>" />            
            </td>

            <td>
                <input  name="autorizacion" type="text" id="autorizacion" class="input_txt " />
            </td>

          <td >
            <input type="text" size="7px" name="id_cup_1" id="id_cup_1"  onchange="traer_valor();"/>
        </td>
      ...
      <td ><input name="copago_v" type="text" id="copago_v" readonly="true" value="0" />
        ...
        <tr>
            ...
            <td >
                ...
                <input type="hidden" id="desvalor" name="desvalor" value="0" />
