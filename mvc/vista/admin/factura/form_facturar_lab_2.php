...
$comision=0;
 ...
 $id_cita_origen=$_REQUEST['id_cita'];

// agregar que la fecha sea la de hoy


 //preguntar por donde viene el origen si es de refacturar

 if($id_cita_origen !=0){ //refacturar con cita
     $max_factura = "Select max(id_factura) as id_factura from factura where id_usuario='{$_REQUEST['identificacion']}' and  origen_factura=0 and id_cita='{$_REQUEST['id_cita']}'";
           
 }
 else{  // refacturar sin cita
     $max_factura = "Select max(id_factura) as id_factura from factura where id_usuario='{$_REQUEST['identificacion']}' ";
 }
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.bool) {
                    ...
                    alert("seleccione un codigo valido cups para el contrato")
                    limpiar()
                }
            }, error: function(request, error) {
...
<form id="id_factura_formulario" action="?option=detalle_factura&origen=guardar" method="post">
    ...
    <table  height="52" border="1" align="center"   cellpadding="0" cellspacing="0">
        ...
        <tr>
            <td width="152" height="22">PROFESIONAaaaaL:</td>
