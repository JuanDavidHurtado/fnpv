...
foreach ($ids as $id) {
    ...
    if ($id != '') {
        $Str_SQL = "INSERT INTO factura_cup (`id_factura`,`id_usuario`,`id_cup`,`nombre_cup`,`tarifa`,`cantidad`,`valor_t`,`fecha`,`autorizacion`,`autorizacion_2`,`fecha_consulta`)
              VALUES(
              " . $idfactura2 . ",'" . $id_pacie . "','" . $_REQUEST['hdncodigo_' . $id] . "'
                  ,'" . $_REQUEST['hdnnombre_' . $id] . "'," . $_REQUEST['hdnvaloru_' . $id] . ",
                  " . $_REQUEST['hdncantidad_' . $id] . "," . $_REQUEST['hdntotal_' . $id] . ",'
          " . $fecha_factura . "','" . $_REQUEST['auto_' . $id] . "','" . $_REQUEST['auto2_' . $id] . "','" . $_REQUEST['hdniva_' . $id] . "' )";
