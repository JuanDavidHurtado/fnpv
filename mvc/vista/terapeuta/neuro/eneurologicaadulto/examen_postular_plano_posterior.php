<?php
if (isset($_REQUEST['guardar_ner_postural_posterior']) && isset($_REQUEST['id_cita'])) {

    if ($_REQUEST['id_ner_postural_posterior'] == 'new') {

        $sql = "INSERT INTO ner_postural_posterior (
`id_ner_postural_posterior`,
`id_usuario`,
`historia`,
`id_proceso`,
`cabeza11`,
...
'" . $_REQUEST['pies38'] . "'
    ...
    } else if ($_REQUEST['id_ner_postural_posterior'] != '') {

        $sql = "UPDATE ner_postural_posterior set
...
`pies38` = '" . $_REQUEST['pies38'] . "' WHERE `id_ner_postural_posterior` = '" . $_REQUEST['id_ner_postural_posterior'] . "' ;";
    }

    $conexion->EjecutarQuery($sql);
}
$id_ner_postural_posterior = 'new';
$sql = "SELECT * from ner_postural_posterior WHERE `id_vinicial` = '" . $_REQUEST['id_vinicial'] . "' and `id_revaloracion` = '" . $_REQUEST['id_revaloracion'] . "' ";
$result = $conexion->EjecutarQuery($sql);
while ($row = mysql_fetch_array($result)) {
    $id_ner_postural_posterior = $row['id_ner_postural_posterior'];
...
<script>
    ...
    $(function() {
        ...
        $("#close-ner_postural_posterior").click(function() {
            $("#open-ner_postural_posterior").show();
            $(this).hide();
            $("#content-ner_postural_posterior").hide('slow');
        });

        $("#open-ner_postural_posterior").click(function() {
            $("#close-ner_postural_posterior").show();
            $(this).hide();
            $("#content-ner_postural_posterior").show('slow');
        });

    });

</script>
<fieldset id="ner_postural_posterior">
    <legend align="left"> <div class="arrow-c" id="close-ner_postural_posterior"></div> <div class="arrow-o" id="open-ner_postural_posterior" ></div> <font>Valoracion fisioterapia de neurorehabilitacion - Examen postural plano posterior</font></legend>
    <div id="content-ner_postural_posterior">
        <?php
        if (isset($_REQUEST['id_cita'])) {
            $button = '<button name="guardar_ner_postural_posterior" id="guardar_ner_postural_posterior" >Guardar</button>';
        } else {
            $readonly = " readonly='readonly' ";
        }
        ?>
        <form id="form_ner_postural_posterior" name="form_ner_postural_posterior" action="<?php echo @$action_form_ner_postural_posterior ?>#ner_postural_posterior" method="post">

            <input type="hidden" id="id_ner_postural_posterior" name="id_ner_postural_posterior" value="<?php echo @$id_ner_postural_posterior ?>" />


            <input type="hidden" class="input_txt valid" id="id_ner_postural_posterior" name="id_ner_postural_posterior" value="<?php echo @$id_ner_postural_posterior; ?>" />
