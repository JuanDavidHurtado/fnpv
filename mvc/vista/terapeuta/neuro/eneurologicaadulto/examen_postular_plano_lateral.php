<?php
if (isset($_REQUEST['guardar_ner_plano_lateral']) && isset($_REQUEST['id_cita'])) {

    if ($_REQUEST['id_ner_plano_lateral'] == 'new') {

        $sql = "INSERT INTO ner_plano_lateral (
`id_ner_plano_lateral`,
`id_usuario`,
`historia`,
`id_proceso`,
`c1l`,
...
'" . $_REQUEST['ppie8l'] . "'
    ...
    } else if ($_REQUEST['id_ner_plano_lateral'] != '') {

       $sql = "UPDATE ner_plano_lateral set
...
`ppie7l` = '" . $_REQUEST['id_cita'] . "' ,
`ppie7l` = '" . $_REQUEST['id_vinicial'] . "' ,
`ppie8l` = '" . $_REQUEST['ppie8l'] . "' WHERE `id_ner_plano_lateral` = '" . $_REQUEST['id_ner_plano_lateral'] . "' ;";
    }

    $conexion->EjecutarQuery($sql);
}
$id_ner_plano_lateral = 'new';
$sql = "SELECT * from ner_plano_lateral WHERE `id_vinicial` = '" . $_REQUEST['id_vinicial'] . "' and `id_revaloracion` = '" . $_REQUEST['id_revaloracion'] . "'  ";
$result = $conexion->EjecutarQuery($sql);
while ($row = mysql_fetch_array($result)) {
    $id_ner_plano_lateral = $row['id_ner_plano_lateral'];
...
<script>
    ...
    $(function() {
        ...
        $("#close-ner_plano_lateral").click(function() {
            $("#open-ner_plano_lateral").show();
            $(this).hide();
            $("#content-ner_plano_lateral").hide('slow');
        });

        $("#open-ner_plano_lateral").click(function() {
            $("#close-ner_plano_lateral").show();
            $(this).hide();
            $("#content-ner_plano_lateral").show('slow');
        });

    });

</script>
<fieldset id="ner_plano_lateral">
    <legend align="left"> <div class="arrow-c" id="close-ner_plano_lateral"></div> <div class="arrow-o" id="open-ner_plano_lateral" ></div> <font>Valoracion fisioterapia de neurorehabilitacion - Examen postural plano lateral</font></legend>
    <div id="content-ner_plano_lateral">
<?php
if (isset($_REQUEST['id_cita'])) {
    $button = '<button name="guardar_ner_plano_lateral" id="guardar_ner_plano_lateral" >Guardar</button>';
} else {
    $readonly = " readonly='readonly' ";
}
?>
        <form id="form_ner_plano_lateral" name="form_ner_plano_lateral" action="<?php echo @$action_form_ner_plano_lateral ?>#ner_plano_lateral" method="post">

            <input type="hidden" id="id_ner_plano_lateral" name="id_ner_plano_lateral" value="<?php echo @$id_ner_plano_lateral ?>" />

            <input type="hidden" class="input_txt valid" id="id_ner_plano_lateral" name="id_ner_plano_lateral" value="<?php echo @$id_ner_plano_lateral; ?>" />
