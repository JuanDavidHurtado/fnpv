<?php
if (isset($_REQUEST['guardar_h2_proceso_mental']) && isset($_REQUEST['id_cita'])) {

    if ($_REQUEST['id_h2_proceso_mental'] == 'new') {

        $sql = "INSERT INTO h2_proceso_mental (
`id_h2_proceso_mental`,
`id_usuario`,
`historia`,
`id_proceso`,
`aten_1`,
...
'" . $_REQUEST['sub_ejecu'] . "'
    ...
    } else if ($_REQUEST['id_h2_proceso_mental'] != '') {

        $sql = "UPDATE h2_proceso_mental set
...
`sub_ejecu` = '" . $_REQUEST['sub_ejecu'] . "' WHERE `id_h2_proceso_mental` = '" . $_REQUEST['id_h2_proceso_mental'] . "' ;";
    }

    $conexion->EjecutarQuery($sql);
}
$id_h2_proceso_mental = 'new';
$sql = "SELECT * from h2_proceso_mental WHERE `id_vinicial` = '" . $_REQUEST['id_vinicial'] . "' and `id_revaloracion` = '" . $_REQUEST['id_revaloracion'] . "' ";
$result = $conexion->EjecutarQuery($sql);
while ($row = mysql_fetch_array($result)) {
    $id_h2_proceso_mental = $row['id_h2_proceso_mental'];
...
<script>
    ...
    $(function() {
        ...
        $("#close-h2_proceso_mental").click(function() {
            $("#open-h2_proceso_mental").show();
            $(this).hide();
            $("#content-h2_proceso_mental").hide('slow');
        });

        $("#open-h2_proceso_mental").click(function() {
            $("#close-h2_proceso_mental").show();
            $(this).hide();
            $("#content-h2_proceso_mental").show('slow');
        });

    });

</script>
<fieldset id="h2_proceso_mental">
    <legend align="left"> <div class="arrow-c" id="close-h2_proceso_mental"></div> <div class="arrow-o" id="open-h2_proceso_mental" ></div> <font>Valoracion terapia ocupacional neurorehabilitacion - Procesos mentales</font></legend>
    <div id="content-h2_proceso_mental">
        <?php
        if (isset($_REQUEST['id_cita'])) {
            $button = '<button name="guardar_h2_proceso_mental" id="guardar_h2_proceso_mental" >Guardar</button>';
        } else {
            $readonly = " readonly='readonly' ";
        }
        ?>
        <form id="form_h2_proceso_mental" name="form_h2_proceso_mental" action="<?php echo @$action_form_h2_proceso_mental ?>#h2_proceso_mental" method="post">

            <input type="hidden" id="id_h2_proceso_mental" name="id_h2_proceso_mental" value="<?php echo @$id_h2_proceso_mental ?>" />
            <p><strong>COGNICIÓN</strong></p>
            <table  border="0" cellspacing="0" cellpadding="0" class="data" width="795">
                <tr>
            ...
            <p><strong>ACTIVIDADES PRODUCTIVAS DE OCIO Y TIEMPO LIBRE</strong><br />
                ...
                <fieldset>
                    <legend align="left">
                        <h3>MODELO DE LA OCUPACION HUMANA (MOH)</h3>
                    </legend>
