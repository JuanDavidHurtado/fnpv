...
if (isset($_REQUEST['guardar_h1_meca_psotural_nino']) && isset($_REQUEST['id_cita'])) {

    if ($_REQUEST['id_h1_meca_postural_nino'] == 'new') {

        $sql = "INSERT INTO h1_meca_psotural_nino (
`id_h1_meca_postural_nino`,
`id_usuario`,
`historia`,
`id_proceso`,
`apoyo_pd`,
...
'" . $_REQUEST['protec_aui'] . "' ,
'" . $_REQUEST['postu_apli'] . "' ,
'" . $_REQUEST['postu_noapli'] . "' ,
'" . $_REQUEST['id_cita'] . "' ,
'" . $_REQUEST['id_vinicial'] . "' ,
'" . $_REQUEST['id_revaloracion'] . "' ,
'" . $_REQUEST['obser_postu_16'] . "'
) ";
    } else if ($_REQUEST['id_h1_meca_postural_nino'] != '') {

        $sql = "UPDATE h1_meca_psotural_nino set
...
`protec_aui` = '" . $_REQUEST['protec_aui'] . "' ,
`postu_apli` = '" . $_REQUEST['postu_apli'] . "' ,
`postu_noapli` = '" . $_REQUEST['postu_noapli'] . "' ,
`obser_postu_16` = '" . $_REQUEST['obser_postu_16'] . "' WHERE `id_h1_meca_postural_nino` = '" . $_REQUEST['id_h1_meca_postural_nino'] . "' ;";
    }

    $conexion->EjecutarQuery($sql);
}
$id_h1_meca_postural_nino = 'new';
$sql = "SELECT * from h1_meca_psotural_nino WHERE `id_vinicial` = '" . $_REQUEST['id_vinicial'] . "' and `id_revaloracion` = '" . $_REQUEST['id_revaloracion'] . "' ";
$result = $conexion->EjecutarQuery($sql);
while ($row = mysql_fetch_array($result)) {
    $id_h1_meca_postural_nino = $row['id_h1_meca_postural_nino'];
    ...
    $postu_apli = $row['postu_apli'];
    $postu_noapli = $row['postu_noapli'];
    $obser_postu_16 = $row['obser_postu_16'];
    break;
}
?>
<script>
    ...
    $(function() {
        ...
        $("#close-h1_meca_psotural_nino").click(function() {
            $("#open-h1_meca_psotural_nino").show();
            $(this).hide();
            $("#content-h1_meca_psotural_nino").hide('slow');
        });

        $("#open-h1_meca_psotural_nino").click(function() {
            $("#close-h1_meca_psotural_nino").show();
            $(this).hide();
            $("#content-h1_meca_psotural_nino").show('slow');
        });

    });

</script>
<fieldset id="h1_meca_psotural_nino">
    <legend align="left"> <div class="arrow-c" id="close-h1_meca_psotural_nino"></div> <div class="arrow-o" id="open-h1_meca_psotural_nino" ></div> <font>Valoracion fisioterapia de neurodesarrollo - Mecanismos de control postular</font></legend>
    <div id="content-h1_meca_psotural_nino">
        <?php
        if (isset($_REQUEST['id_cita'])) {
            $button = '<button name="guardar_h1_meca_psotural_nino" id="guardar_h1_meca_psotural_nino" >Guardar</button>';
        } else {
            $readonly = " readonly='readonly' ";
        }
        ?>
        <form id="form_h1_meca_psotural_nino" name="form_h1_meca_psotural_nino" action="<?php echo @$action_form_h1_meca_psotural_nino ?>#h1_meca_psotural_nino" method="post">

            <input type="hidden" id="id_h1_meca_postural_nino" name="id_h1_meca_postural_nino" value="<?php echo @$id_h1_meca_postural_nino ?>" />

            <p><strong>REACCIONES DE  EQUILIBRIO Y PROTECTIVAS:</strong></p>
            <p>&nbsp;</p>
            <p><strong>Aplica:
                    <input type="text" class="input_txt valid" id="postu_apli" name="postu_apli" value="<?php echo @$postu_apli; ?>" />
                    No aplica:
                    <input type="text" class="input_txt valid" id="postu_noapli" name="postu_noapli" value="<?php echo @$postu_noapli; ?>" />
                </strong></p>
            ...
            <p><strong>OBSERVACIONES:
                    <input type="text" class="input_txt valid" id="obser_postu_16" name="obser_postu_16" value="<?php echo @$obser_postu_16; ?>" />
                </strong></p>
            <?php echo @$button ?>
        </form><br>
    </div>
