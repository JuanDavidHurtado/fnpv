<?php
if (isset($_REQUEST['guardar_funcionalidad_nino']) && isset($_REQUEST['id_cita'])) {

    if ($_REQUEST['id_funcionalidad_nino'] == 'new') {

        $sql = "INSERT INTO funcionalidad_nino (
`id_funcionalidad_nino`,
`id_proceso`,
`id_usuario`,
`historia`,
`fuerza`,
...
'" . $_REQUEST['t200'] . "' 
    ...
    } else if ($_REQUEST['id_funcionalidad_nino'] != '') {

        $sql = "UPDATE funcionalidad_nino set 
...
`t198` = '" . $_REQUEST['t198'] . "' ,
`t199` = '" . $_REQUEST['t199'] . "' ,
`t200` = '" . $_REQUEST['t200'] . "' WHERE `id_funcionalidad_nino` = '" . $_REQUEST['id_funcionalidad_nino'] . "' ;";
    }

    $conexion->EjecutarQuery($sql);
}
$id_funcionalidad_nino = 'new';
$sql = "SELECT * from funcionalidad_nino WHERE `id_vinicial` = '" . $_REQUEST['id_vinicial'] . "' and `id_revaloracion` = '" . $_REQUEST['id_revaloracion'] . "' ";
$result = $conexion->EjecutarQuery($sql);
while ($row = mysql_fetch_array($result)) {
    $id_funcionalidad_nino = $row['id_funcionalidad_nino'];
...
<script>
    ...
    $(function() {
        ...
        $("#close-funcionalidad_nino").click(function() {
            $("#open-funcionalidad_nino").show();
            $(this).hide();
            $("#content-funcionalidad_nino").hide('slow');
        });

        $("#open-funcionalidad_nino").click(function() {
            $("#close-funcionalidad_nino").show();
            $(this).hide();
            $("#content-funcionalidad_nino").show('slow');
        });

    });

</script>
<fieldset id="funcionalidad_nino">
    <legend align="left"> <div class="arrow-c" id="close-funcionalidad_nino"></div> <div class="arrow-o" id="open-funcionalidad_nino" ></div> <font>Valoracion terapia ocupacional de neurodesarrollo - Funcionalidad</font></legend>
    <div id="content-funcionalidad_nino">
        <?php
        if (isset($_REQUEST['id_cita'])) {
            $button = '<button name="guardar_funcionalidad_nino" id="guardar_funcionalidad_nino" >Guardar</button>';
        } else {
            $readonly = " readonly='readonly' ";
        }
        ?>
        <form id="form_funcionalidad_nino" name="form_funcionalidad_nino" action="<?php echo @$action_form_funcionalidad_nino ?>#funcionalidad_nino" method="post">

            <input type="hidden" id="id_funcionalidad_nino" name="id_funcionalidad_nino" value="<?php echo @$id_funcionalidad_nino ?>" />
            ...
            <table border="0" class='data text_small' cellspacing="0" cellpadding="0" width="642">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td width="108" valign="top"><p align="center">Abducción Horizontal</p></td>
