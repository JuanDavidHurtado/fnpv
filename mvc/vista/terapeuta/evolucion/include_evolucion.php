...
$sql = "SELECT numero_sesiones FROM `valoracion_inicial` WHERE `id_vinicial` = '" . $_REQUEST['id_vinicial'] . "' ";
...
if ($cantidad_sessiones_det > 0) {
    ...
    $cant = 1;
    if ($cantidad_sessiones_ejec < $cantidad_sessiones_det) {
    ...
    for ($i = 1; $i <= ($cant); $i++) {
        ...
        if ($i == $cantidad_sessiones_det) {
            $bool_final = true;
            $_REQUEST['label_evolution'] = 'Evolucion valoracion final';
        } else {
