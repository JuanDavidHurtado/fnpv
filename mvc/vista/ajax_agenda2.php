    ...
    if ($_REQUEST['opcion'] == 7) {
        ...
        if ($_REQUEST['fecha'] >= $date) {
            ...
            if (!$bool_c) {
                ...
                $deseada=$_REQUEST['fecha_des'];

                $sql_insert_cita = "INSERT INTO `cita` 
                                    (`fecha_hoy`, `id_proceso`, `id_usuario`, `id_paciente`, `fecha`, `fecha_hora_inicio`, `fecha_hora_fin`,`fecha_hora_limite`, `id_sesion`,`id_usuario_agenda`, `tipo_cita`,`fecha_des`, `id_agenda`)
                                    VALUES ('$fecha_hoy', '" . $_REQUEST['id_proceso'] . "', '" . $_REQUEST['id_usuario'] . "', '" . $_REQUEST['id_paciente'] . "', '" . $_REQUEST['fecha'] . "', '$fecha_hora_inicio', '$fecha_hora_fin', '$fecha_hora_fin', '" . $_REQUEST['id_sesion'] . "','" . $_SESSION['id_usuario'] . "','" . $_REQUEST['tipo_cita'] . "', '$deseada', '{$_REQUEST['idAgenda']}')";
