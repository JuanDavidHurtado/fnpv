    ...
    if ($_REQUEST['opcion'] == 1) {
                ...
                while ($row_s_i = mysql_fetch_array($result_s_i)) {
                    ...
                    if ($row_s_i['tipo'] == 1) {
                        ...
                        foreach ($array_p_i as $row_pi) {
                            ...
                            $return .= "<td class='$style_c' align='center' $click style='cursor:pointer'>$msg</td>";
                        }
                    } else {
       ...
       //    and c.fecha='" . $_REQUEST['fecha'] . "'
         ...
         $to = trim($_REQUEST['to']);
           ...
           $sql = "Select * from cita where id_paciente=".  $row['historia']."  $where order by fecha desc"  ;
       ...
       // buscar el proceso
        ...
        while ($row_c = mysql_fetch_array($result_c)) {
            ...
            $sql_pa = "Select * from paciente where historia = " . $row_c['id_paciente'];
                       ...
                       $click = "onclick='verificar_cita_x(\"" . $row_pa['identificacion'] . "\",$id_cita)'";
            ...
            $sql = "Select d.nombre as dependencia, p.nombre as proceso from proceso as p inner join dependencia as d on p.id_dependencia = d.id_dependencia
                where p.id_proceso = '" . $row_c['id_proceso'] . "' ";
        $res = $conexion->EjecutarQuery($sql);
        $row1 = mysql_fetch_array($res);


        $sql23 = "Select * from usuario where id_usuario = '" . $row_c['id_usuario'] . "' ";
$result23 = $conexion->EjecutarQuery($sql23);
$row23 = mysql_fetch_array($result23);
$nombre_usuario = $row23['nombre'] . " " . $row23['apellido'];

            $return .= '<tr class="' . $style_c . '" ' . $click . ' >';
            $return .= '<td align="center">' . $row1['proceso'] . '</td>';
            $return .= '<td align="center">' . $row_c['estado'] . '</td>';
            $return .= '<td align="center"> ' . ($row_c['fecha_hora_inicio']) . '</td>';
            $return .= '<td align="center">' . $row23['registro_medico'] . '</td>';
            $return .= '<td align="center">' . $nombre_usuario . '</td>';
            $return .= '</tr>';
        }
