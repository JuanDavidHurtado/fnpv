...
class EasyGoogleMap_1 {
	...
	function EasyGoogleMap_1($mapKey) {
		$this->mMapKey = $mapKey;
		$this->SetMapWidth();
		$this->SetMapHeight();
		$this->SetMapZoom();
	...
	function InitJs() {
		...
		if ($is_error) { 
			...
			$ret .= "	var $this->idMap = new GMap2(document.getElementById('$this->idMap')); \n";
	
			# handle map continuous zooming
			$ret .= ($this->mContinuousZoom==TRUE)?"	$this->idMap.enableContinuousZoom(); \n":"";
	
			# handle map double click zooming
			$ret .= ($this->mDoubleClickZoom==TRUE)?"	$this->idMap.enableDoubleClickZoom(); \n":"";
			...
			switch ($this->mDefControl) {
				...
				case 'SMALL_PAN_ZOOM':
					$mapCtrl = "$this->idMap.addControl(new GSmallMapControl()); \n";
					break;
					
				case 'LARGE_PAN_ZOOM':
					$mapCtrl = "$this->idMap.addControl(new GLargeMapControl()); \n";
					break;
	
				case 'SMALL_ZOOM':
					$mapCtrl = "$this->idMap.addControl(new GSmallZoomControl()); \n";
					break;
			...
			$ret .= ($this->mScale==TRUE)?"	$this->idMap.addControl(new GScaleControl()); \n":"";
	
			# handle map type (map/satellite/hybrid)
			$ret .= ($this->mMapType==TRUE)?"	$this->idMap.addControl(new GMapTypeControl()); \n":"";
	
			# handle map inset
			$ret .= ($this->mInset==TRUE)?"	$this->idMap.addControl(new GOverviewMapControl()); \n":"";
	
			$ret .= "	var geocoder2 = new GClientGeocoder(); \n";
			$ret .= "	var icon2 = new GIcon(); \n";
			$ret .= "	icon2.image = 'http://google.webassist.com/google/markers/$dir/$color.png'; \n";
			$ret .= "	icon2.shadow = 'http://google.webassist.com/google/markers/$dir/shadow.png'; \n";
			$ret .= "	icon2.iconSize = new GSize($icon_w,$icon_h); \n";
			$ret .= "	icon2.shadowSize = new GSize($icon_w,$icon_h); \n";
			$ret .= "	icon2.iconAnchor = new GPoint($icon_anchr_w,$icon_anchr_h); \n";
			$ret .= "	icon2.infoWindowAnchor = new GPoint($info_win_anchr_w,$info_win_anchr_h); \n";
			$ret .= "	icon2.printImage = 'http://google.webassist.com/google/markers/$dir/$color.gif'; \n";
			$ret .= "	icon2.mozPrintImage = 'http://google.webassist.com/google/markers/$dir/{$color}_mozprint.png'; \n";
			$ret .= "	icon2.printShadow = 'http://google.webassist.com/google/markers/$dir/shadow.gif'; \n";
			$ret .= "	icon2.transparent = 'http://google.webassist.com/google/markers/$dir/{$color}_transparent.png'; \n\n";
			...
			for ($i=$cnt_add-1; $i>=0; $i--) {
				...
				$ret .= "	address2[$i] = address_2$i.infowindowtext;\n\n";
				
				$ret .= "	geocoder2.getLatLng (\n";
				$ret .= "	  address_2$i.full,\n";
				$ret .= "	  function(point2) {\n";
				$ret .= "		if(point2) {\n";
				$ret .= "		  points2[$i] = point2; \n";
				$ret .= "		  $this->idMap.setCenter(point2, {$this->mMapZoom});\n";
				$ret .= "		  var marker2 = new GMarker(point2, icon2);\n";
				$ret .= "		  GEvent.addListener(marker2, 'click', function() {\n";
				$ret .= "			marker2.openInfoWindowHtml(address_2$i.infowindowtext);\n";
				$ret .= "		  });\n";

				$ret .= "		  $this->idMap.addOverlay(marker2);\n";

				# show only info window to the first set address
				if ($i===0) 
					$ret .= "		  marker2.openInfoWindowHtml(address_2$i.infowindowtext);\n";
				
				$ret .= "		  gmarkers2[$i] = marker2;\n";

				$ret .= "		}\n";
				$ret .= "		else {\n";
				$ret .= "		  $this->idMap.setCenter(new GLatLng(37.4419, -122.1419), {$this->mMapZoom});\n";
				$ret .= "		}\n";
				$ret .= "	  }\n";
			...
			$ret .= "   if (gmarkers2[i]) {\n";
			$ret .= "	  gmarkers2[i].openInfoWindowHtml(address2[i]);\n";
			$ret .= "	  $this->idMap.setCenter(points2[i],{$this->mMapZoom});\n";
			$ret .= "   } else {\n";
			$ret .= "	  var htstring2 = address2[i];\n";
			$ret .= "	  var stripped2 = htstring2.replace(/(<([^>]+)>)/ig,'');\n";
	 ...
	 */
	function MapHolder() {
		return "<div id=\"$this->idMap\" style=\"width: ".$this->mMapWidth."px; height: ".$this->mMapHeight."px;\"></div>";
	} # end function
